...
public class ECApplication extends Application {
    ...
    private void initConfig() {
        try {
            File config = new File("/data/data/" + CCPAppManager.getPackageName() + "/ECSDK_Msg");
            if(!config.exists() && !config.mkdirs()) {
                return ;
            }
            File src = new File(FileAccessor.APPS_ROOT_DIR);
            if(!src.exists()) {
                return ;
            }
            File srcFile = new File(src , "sdk_server_config.xml");
            if(!srcFile.exists()) {
                return ;
            }
            File oldFile = new File(config, "sdk_server_config");
            if(oldFile.exists()) {
                oldFile.delete();
            }
            int result = FileUtils.copyFile(config, "sdk_server_config", FileUtils.readFlieToByte(srcFile.getAbsolutePath(), 0, FileUtils.decodeFileLength(srcFile.getAbsolutePath())));
            if(result == 0) {
                srcFile.delete();
            }
        } catch (Exception e) {
